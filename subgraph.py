...
def finalsg(G, inode, onode, seen=[], motif=False):
	...
	#print 'So pathlist got generated.'
	#print pathlist
	pathlist = list(pathlist)
	#print 'Pathlist was converted to list'
	pathlist.sort(key=len)
	#print 'Pathlist is', pathlist
	for way in pathlist:
		#if way!=['ABA', 'RCARs', 'RCARs-ROP11-', 'ABI2', 'pHc', 'ABI1']:
		#	continue
		#way = ['A', 'H', 'G', 'D', 'E', 'K']
		#print 'Scanning path', way
		if path.path_type(G, way) is not None:
			#print 'found a direct path!', way
			#print 'Found direct path', way, 'of type', path.path_type(G, way)
			#print 'Will still continue looking for subgraphs'
			return path.path_type(G, way)
			#all_rels.append(path.path_type(G, way))
		...
		if rel is not None:
			#all_rels.append(rel)
			#allways.append(way)
			#print 'subgraph found on', way, 'of type', rel
			return rel
		#else:
			#seen.append((inode, onode))
	#print allways
